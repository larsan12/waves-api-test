...
(async () => {
    ...
    try {
      const result = await Waves.API.Node.transactions.sign('dockerCallV2', data, seed.keyPair);
      console.log('Success!')
      // Готовая транзакция, отправлять на ноду в эндпоинт /transactions/broadcast
      console.log(result)
    } catch (err) {
